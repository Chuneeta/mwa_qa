...
class TestVisMetrics(unittest.TestCase):
	...
	def test_initialize_metrics_dict(self):
		...
		self.assertEqual(list(vis.metrics[128].keys()), ['XX', 'XY', 'YX', 'YY'])
	...
	def test_run_metrics(self):
		vis = vm.VisMetrics(uvfits)
		vis.run_metrics()
		self.assertEqual(list(vis.metrics['XX']['autos']['Tile011'].keys()), ['mean_amp_freq', 'median_amp_freq', 'rms_amp_freq', 'var_amp_freq'])
		np.testing.assert_almost_equal(vis.metrics[1]['XX']['autos']['mean_amp_freq'], 943.5318914370374, decimal = 2)
		np.testing.assert_almost_equal(vis.metrics[1]['XX']['autos']['median_amp_freq'], 885.4816159565241, decimal = 2)
		np.testing.assert_almost_equal(vis.metrics[1]['XX']['autos']['rms_amp_freq'], 967.5731931584965, decimal = 2)
		np.testing.assert_almost_equal(vis.metrics[1]['XX']['autos']['var_amp_freq'], 45945.45396017565, decimal = 2)
