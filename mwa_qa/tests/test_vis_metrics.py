...
uvfits_path = os.path.join(DATA_PATH, '1062784992.uvfits')
...
class TestVisMetrics(unittest.TestCase):
    def test__init__(self):
        vis = VisMetrics(uvfits_path)
        self.assertTrue(vis.uvfits_path, uvfits_path)
    ...
    def test_write_to(self):
        vis = VisMetrics(uvfits_path)
        vis._initialize_metrics_dict()
        outfile = uvfits_path.replace('.uvfits', '_vis_metrics.json')
        vis.write_to()
